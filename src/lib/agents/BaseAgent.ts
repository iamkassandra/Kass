...
export abstract class BaseAgent {
  ...
  protected async generateResponse(question: string): Promise<string> {
    try {
      const res = await openAIService.chatWithAgent({
        agentRole: this.role,
        agentName: this.name,
        message: question,
        context: `Agent ID: ${this.id}, Current Workload: ${this.workload}`
      });
      return res.text;
    } catch (error) {
      console.error(`[${this.name}] GPT-4o response generation failed:`, error);
      return `[GPT-4o] ${this.name} processed: "${question}".`;
    }
  }
