...
export class ExecutiveAgent extends BaseAgent {
  ...
  async generatePlan(objective: string): Promise<Task[]> {
    console.log(`[Executive] Generating plan with GPT-4o for: ${objective}`);

    const businessType = this.extractBusinessType(objective);

    try {
      const gptPlan = await openAIService.generateAutonomousProjectPlan({
        objective,
        businessType
      });

      if (gptPlan.tasks && gptPlan.tasks.length > 0) {
        const roleToAgentId: Record<string, string> = {
          executive: 'executive-001',
          engineer: 'engineer-001',
          designer: 'designer-001',
          legal: 'legal-001',
          marketing: 'marketing-001',
          devops: 'devops-001',
          testing: 'testing-001'
        };

        const baseTime = Date.now();
        return gptPlan.tasks.map((t, idx) => ({
          id: `task-${baseTime}-${idx}`,
          title: t.title,
          description: t.description + (t.deliverables ? ` (Deliverables: ${t.deliverables.join(', ')})` : ''),
          status: 'pending' as const,
          priority: (t.priority || 'high') as any,
          assignedAgent: roleToAgentId[t.assignedRole] || 'engineer-001',
          dependencies: idx > 0 ? [`task-${baseTime}-${idx - 1}`] : [],
          createdAt: new Date(),
          updatedAt: new Date()
        }));
      }
    } catch (error) {
      console.warn('[Executive] Fallback to structured planning:', error);
    }
